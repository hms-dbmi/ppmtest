...
from splinter.driver.webdriver.remote import WebDriver as SplinterRemoteDriver
...
def Browser(browser_name='firefox', url=None):
    ...
    elif browser_name.lower() == 'remote_firefox':
        return RemoteFirefox(url=url)

    elif browser_name.lower() == 'remote_chrome':
        return RemoteChrome(url=url)

    elif browser_name.lower() == 'remote_ie':
        return RemoteInternetExplorer(url=url)
...
class BrowserExtensions(BaseWebDriver):
    ...
    def __init__(self, wait_time=5, **kwargs):
        ...
        super(BrowserExtensions, self).__init__(wait_time=wait_time, **kwargs)

    def open_tab(self, name, location=None):

        # Check if no tab has been opened yet.
        if len(self._windows) == 0:

            # Open the location
            self._windows.append(name)

            # Open a location if needed
            if location:
                self.visit(location)
            ...
            # Open a new one
            if location:
                self.driver.execute_script("window.open('{}');".format(location))
            else:
    ...
    def go_to_tab(self, name):
        ...
        if name not in self._windows:
            ...
            # Get the index
            index = self._windows.index(name)
            self.driver.switch_to_window(self.driver.window_handles[index])
...
class Firefox(FirefoxDriver, BrowserExtensions):
    ...
    def __init__(self):
        ...
        driver_path = os.path.join(project_root, os.path.join('drivers', 'geckodriver'))
        kwargs = {'executable_path': driver_path, 'profile_preferences': preferences}

        super(Firefox, self).__init__(wait_time=5, **kwargs)


class RemoteFirefox(SplinterRemoteDriver, BrowserExtensions):
...
class Chrome(ChromeDriver, BrowserExtensions):
    ...
    def __init__(self):
        ...
        # Get the path to the project root.
        project_root = os.path.realpath(os.path.dirname(__file__))
        driver_path = os.path.join(project_root, os.path.join('drivers', 'chromedriver'))
